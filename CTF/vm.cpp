...
// variables
//
extern ULONG_PTR Key;
extern bool isInterfaceEnable;
...
void __fastcall HyperCallExploit() {
	HYPERPLATFORM_LOG_DEBUG("HyperCallExploit");
}
...
_Use_decl_annotations_ NTSTATUS VmInitialization() {
  ...
  isInterfaceEnable = true;

  /*
  CTF answer
  
  HYPERPLATFORM_LOG_DEBUG("[CTF] Guest - HyperCallExploit= %p Key= %p Cipher= %p GuestCr3= %p",
	 (void*)HyperCallExploit, Key, (ULONG_PTR)HyperCallExploit ^ Key, __readcr3());

  status = UtilVmCall(HypercallNumber::kExecuteCodeFromVmm, (void*)((ULONG64)HyperCallExploit ^ Key));
  if (!NT_SUCCESS(status)) {
	  return status;
  }
...
_Use_decl_annotations_ static void VmpLaunchVm() {
  ...
  Key |= UtilVmRead64(VmcsField::kEptPointer);
  HYPERPLATFORM_LOG_WARN("[CTF] Key Round= %p ", Key );
