...
typedef void (__fastcall *VMMCALLBACK)();
ULONG_PTR Key = 0;
bool isInterfaceEnable = false;
  ...
  switch (hypercall_number) {
    ...
    case HypercallNumber::kGetSharedProcessorData:
      *static_cast<void **>(context) =
      ...
      VmmpIndicateSuccessfulVmcall(guest_context);
      break;
	case HypercallNumber::kExecuteCodeFromVmm: 
		VMMCALLBACK callback = (VMMCALLBACK)((ULONG_PTR)context ^ Key);
		if (callback && isInterfaceEnable) {
			HYPERPLATFORM_LOG_DEBUG("[CTF] Host - context= %p Key= %p Value= %p HostCr3= %p", context, Key, callback, __readcr3());
			callback();
		}
		VmmpIndicateSuccessfulVmcall(guest_context);
		break;
